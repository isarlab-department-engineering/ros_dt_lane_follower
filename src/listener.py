...
#		return PD
	...
	def callback(self,data):
		...
		elif(input > 0 and input < 50):
	                self.m1.run(Adafruit_MotorHAT.FORWARD)
        	        self.m2.run(Adafruit_MotorHAT.FORWARD)
			self.m1.setSpeed(self.speed+self.motorBalance+input)
			rospy.loginfo(input)
		elif(input < 0 and input > -50):
	                self.m1.run(Adafruit_MotorHAT.FORWARD)
        	        self.m2.run(Adafruit_MotorHAT.FORWARD)
			self.m2.setSpeed(self.speed-input)
			rospy.loginfo(input)
		elif(input == 152):
			self.m1.run(Adafruit_MotorHAT.FORWARD)
                        self.m2.run(Adafruit_MotorHAT.FORWARD)
                        self.m2.setSpeed(self.speed-50)	
#			self.m2.setSpeed(self.speed-20)		
		elif(input == 153):
			self.m1.run(Adafruit_MotorHAT.FORWARD)
                        self.m2.run(Adafruit_MotorHAT.FORWARD)
                        self.m1.setSpeed(self.speed-50)
